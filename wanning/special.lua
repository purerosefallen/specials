...
addSkill(47660516, function(e2)
	local e1=Effect.GlobalEffect()
	e1:SetDescription(aux.Stringid(13014905,2))
	e1:SetType(EFFECT_TYPE_FIELD)
	e1:SetCode(EFFECT_SPSUMMON_PROC_G)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetRange(LOCATION_MZONE)
	e1:SetOperation(function(e,tp,eg,ep,ev,re,r,rp)
		if not e:GetHandler():IsRankAbove(2) then Duel.SelectOption(tp,aux.Stringid(13014905,2)) end
	...
	e2:SetTarget(function(e,c)
	  return c:GetRank()>0 and c:IsFaceup()
	end)
	e2:SetLabelObject(e1)
end)
...
addSkill(47660516, function(e2)
	local e1=Effect.GlobalEffect()
	e1:SetDescription(aux.Stringid(13014905,3))
	e1:SetType(EFFECT_TYPE_FIELD)
	e1:SetCode(EFFECT_SPSUMMON_PROC_G)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCondition(function (e,c)
		return e:GetHandler():IsRankAbove(2)
	end)
	e1:SetOperation(function(e,tp,eg,ep,ev,re,r,rp)
		local e3=Effect.CreateEffect(e:GetHandler())
		e3:SetType(EFFECT_TYPE_SINGLE)
