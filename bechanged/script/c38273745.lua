...
function c38273745.initial_effect(c)
	...
	e2:SetCondition(c38273745.discon)
	e2:SetTarget(c38273745.distg)
	e2:SetOperation(c38273745.disop)
	c:RegisterEffect(e2)
end
function c38273745.ovfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xa) and c:IsRace(RACE_DRAGON)
end
function c38273745.xyzop(e,tp,chk)
	if chk==0 then return Duel.GetFlagEffect(tp,38273745)==0 end
	Duel.RegisterFlagEffect(tp,38273745,RESET_PHASE+PHASE_END,EFFECT_FLAG_OATH,1)
end
...
function c38273745.discon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsPreviousLocation(LOCATION_MZONE)
		and c:IsPreviousControler(tp) and c:GetReasonPlayer()==1-tp
end
function c38273745.distg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c38273745.disop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(aux.NegateMonsterFilter,tp,0,LOCATION_MZONE,nil)
	local tc=g:GetFirst()
	while tc do
