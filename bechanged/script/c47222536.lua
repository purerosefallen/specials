...
function c47222536.initial_effect(c)
	...
	e1:SetCountLimit(1,47222536+EFFECT_COUNT_CODE_OATH)
	...
	c:RegisterEffect(e3)
	local e4=e2:Clone()
	e4:SetDescription(aux.Stringid(47222536,2))
	e4:SetCategory(CATEGORY_REMOVE+CATEGORY_GRAVE_ACTION)
	e4:SetCondition(c47222536.rmco)
	e4:SetTarget(c47222536.rmt)
	e4:SetOperation(c47222536.rmo)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
...
function c47222536.cfilte(c,tp)
	return c:IsFaceup() and c:IsCode(38033121) and c:IsControler(tp)
end
...
function c47222536.rmco(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c47222536.cfilte,1,nil,tp)
end
function c47222536.rmt(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c47222536.rmo(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) then
		Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)
	end
