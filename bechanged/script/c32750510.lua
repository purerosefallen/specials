...
function c32750510.initial_effect(c)
	...
	e4:SetDescription(aux.Stringid(32750510,2))
	e4:SetCategory(CATEGORY_COUNTER)
	e4:SetType(EFFECT_TYPE_TRIGGER_O+EFFECT_TYPE_SINGLE)
	e4:SetProperty(EFFECT_FLAG_DELAY)
	e4:SetCode(EVENT_SPSUMMON_SUCCESS)
	e4:SetTarget(c32750510.cttg)
	e4:SetOperation(c32750510.ctop)
	c:RegisterEffect(e4)
end
...
function c32750510.spcon(e,c)
	...
	local rg=Duel.GetReleaseGroup(tp,true,REASON_SPSUMMON):Filter(c32750510.rfilter,c,tp)
...
function c32750510.ctfilter2(c)
	return c:IsFaceup() and c:IsCanAddCounter(0x1015,1)
end
function c32750510.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c32750510.ctfilter2,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_OPSELECTED,1-tp,e:GetDescription())
	Duel.SetOperationInfo(0,CATEGORY_COUNTER,nil,1,0,0x1015)
end
function c32750510.ctop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c32750510.ctfilter2,tp,0,LOCATION_MZONE,nil)
	local tc=g:GetFirst()
	while tc do
		tc:AddCounter(0x1015,1)
		tc=g:GetNext()
