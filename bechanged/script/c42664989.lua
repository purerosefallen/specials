...
function c42664989.operation(e,tp,eg,ep,ev,re,r,rp)
	local ct1=Duel.GetTurnCount()-Duel.GetMatchingGroupCount(aux.TRUE,tp,LOCATION_HAND,0,e:GetHandler())+1
	local ct2=Duel.GetTurnCount()-Duel.GetMatchingGroupCount(aux.TRUE,tp,0,LOCATION_HAND,e:GetHandler())+1
	if ct1>6 then ct1=6 end
	if ct2>6 then ct2=6 end
	if ct1>0 then
