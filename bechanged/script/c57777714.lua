...
function c57777714.initial_effect(c)
	...
	e1:SetCountLimit(1,57777714)
	e1:SetTarget(c57777714.sptg)
	e1:SetOperation(c57777714.spop)
	c:RegisterEffect(e1)
...
function c57777714.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c57777714.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) or Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)==0 then return end
	if c:IsType(TYPE_XYZ)
		and Duel.GetMatchingGroupCount(aux.NecroValleyFilter(c57777714.mfilter),tp,LOCATION_GRAVE,0,nil)>0
		and Duel.SelectYesNo(tp,aux.Stringid(57777714,2)) then
		local e1=Effect.CreateEffect(c)
...
function c57777714.filter(c,e,tp)
	return c:IsSetCard(0xb5,0xc4,0x10) and not c:IsCode(57777714) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
