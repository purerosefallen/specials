...
function s.initial_effect(c)
	...
	e7:SetOperation(s.baibai)
	c:RegisterEffect(e7)
end
function s.dabao(e,tp,eg,ep,ev,re,r,rp)
	local t1=Duel.GetFieldGroupCount(tp,0,LOCATION_ONFIELD)
	local s1=Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)
	local t2=Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)
	local s2=Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)
	return t1<=s1 and t2<=s2 
end
function s.baibai(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		e1:SetOperation(s.baibaiop)
		e1:SetLabelObject(g)
		Duel.RegisterEffect(e1,tp)
		g:KeepAlive()
		local tc=g:GetFirst()
		while tc do
			tc:RegisterFlagEffect(71398055,RESET_EVENT+RESETS_STANDARD,0,1)
			tc=g:GetNext()
		end
	end
end
function s.baibaifilter(c)
	return c:GetFlagEffect(71398055)~=0
end
function s.baibaiop(e,tp,eg,ep,ev,re,r,rp)
	local g=e:GetLabelObject()
	local sg=g:Filter(s.baibaifilter,nil)
	g:DeleteGroup()
