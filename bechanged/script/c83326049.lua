...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local types={1057,1074,1076,1077}
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if opt==1074 then ct=TYPE_PENDULUM   p=2 end
	if opt==1076 then ct=TYPE_LINK  p=3 end
	if opt==1077 then ct=(not TYPE_EFFECT)	  p=4 end
	local alist=Duel.GetFlagEffectLabel(tp,id)
	if not alist then
		alist=1<<p
