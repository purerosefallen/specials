...
function c24861088.chainfilter(re,tp,cid)
	return (re:GetHandler():IsSetCard(0x70) or re:GetHandler():IsSetCard(0x95))
	or not re:GetHandler():IsOnField()
end
function c24861088.aclimit(e,re,tp)
	return not (re:GetHandler():IsSetCard(0x70) or re:GetHandler():IsSetCard(0x95))
	and (re:GetHandler():IsOnField() or (re:GetHandler():IsType(TYPE_SPELL+TYPE_TRAP) and re:GetHandler():IsLocation(LOCATION_HAND)))
end
