...
function c79447365.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if (not tc:IsRelateToEffect(e)) or tc:IsFacedown()
	then return end
	local lv=0
	...
	if Duel.IsExistingMatchingCard(c79447365.spfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,e,tp,lv)
	and (
	(not tc:IsControlerCanBeChanged() and tc:IsControler(1-tp))
	or
	Duel.SelectYesNo(tp,aux.Stringid(79447365,1))
	) then
