...
function c15083304.initial_effect(c)
	...
	e4:SetDescription(aux.Stringid(15083304,0))
	e4:SetType(EFFECT_TYPE_SINGLE)
	e4:SetCode(EFFECT_TRAP_ACT_IN_HAND)
	e4:SetCost(c15083304.cost)
	c:RegisterEffect(e4)
end
function c15083304.cfilter(c)
	return c:IsDiscardable()
end
function c15083304.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c15083304.cfilter,tp,LOCATION_HAND,0,1,nil)
		and Duel.IsPlayerCanDraw(tp,3) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
	local g=Duel.SelectMatchingCard(tp,c15083304.cfilter,tp,LOCATION_HAND,0,1,1,nil)
