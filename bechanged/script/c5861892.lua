...
function c5861892.initial_effect(c)
	...
	e11:SetCondition(c5861892.spcon1)
	e11:SetTarget(c5861892.sptg1)
	e11:SetOperation(c5861892.spop1)
...
function c5861892.rfilter(c,tp)
	return c:IsSetCard(0x5)
end
function c5861892.fselect(g,tp)
	return g:IsExists(c5861892.rfilter,1,nil,tp) and aux.mzctcheckrel(g,tp,REASON_SPSUMMON)
end
function c5861892.spcon1(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local mg=Duel.GetMatchingGroup(c5861892.spfilter,tp,LOCATION_MZONE,0,nil)
	return mg:CheckSubGroup(c5861892.fselect,3,3,tp)
end
function c5861892.sptg1(e,tp,eg,ep,ev,re,r,rp,chk,c)
	local mg=Duel.GetMatchingGroup(c5861892.spfilter,tp,LOCATION_MZONE,0,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local sg=mg:SelectSubGroup(tp,c5861892.fselect,true,3,3,tp)
	if sg then
		sg:KeepAlive()
		e:SetLabelObject(sg)
		return true
...
function c5861892.spop1(e,tp,eg,ep,ev,re,r,rp,c)
	local g=e:GetLabelObject()
	Duel.SendtoGrave(g,REASON_SPSUMMON)
	g:DeleteGroup()
end
