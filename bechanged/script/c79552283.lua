...
function c79552283.initial_effect(c)
	...
	e5:SetDescription(aux.Stringid(79552283,2))
	e5:SetCategory(CATEGORY_DISABLE)
	e5:SetType(EFFECT_TYPE_QUICK_O)
	e5:SetCode(EVENT_CHAINING)
	e5:SetRange(LOCATION_SZONE)
	e5:SetCountLimit(1,79552283)
	e5:SetCondition(c79552283.discon)
	e5:SetCost(c79552283.discost2)
	e5:SetTarget(c79552283.distg)
	e5:SetOperation(c79552283.disop)
	c:RegisterEffect(e5)
end
...
function c79552283.adatkop(e,tp,eg,ep,ev,re,r,rp)
	...
	e2:SetCondition(c79552283.damconatk)
	e2:SetValue(c79552283.damval)
	e2:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
	tc:RegisterEffect(e2)
	local e3=Effect.CreateEffect(e:GetHandler())
	e3:SetType(EFFECT_TYPE_SINGLE)
	e3:SetCode(EFFECT_SET_DEFENSE_FINAL)
	e3:SetCondition(c79552283.damcondef)
	e3:SetValue(c79552283.damval)
	e3:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
	tc:RegisterEffect(e3)
end

function c79552283.damfilter(c,e,tp)
...
function c79552283.damconatk(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return (Duel.GetCurrentPhase()==PHASE_DAMAGE or Duel.GetCurrentPhase()==PHASE_DAMAGE_CAL) and Duel.IsExistingMatchingCard(c79552283.damfilter,tp,LOCATION_MZONE,0,1,nil) and (c:GetAttack()>c:GetDefense())
end
function c79552283.damcondef(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return (Duel.GetCurrentPhase()==PHASE_DAMAGE or Duel.GetCurrentPhase()==PHASE_DAMAGE_CAL) and Duel.IsExistingMatchingCard(c79552283.damfilter,tp,LOCATION_MZONE,0,1,nil) and (c:GetAttack()<c:GetDefense())
end
function c79552283.damval(e)
	local c=e:GetHandler()
	return math.max(c:GetAttack(),c:GetDefense())+1000
end
...
function c79552283.tgfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x17a) and c:IsType(TYPE_MONSTER)
end
...
function c79552283.discost2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c79552283.tgfilter,tp,LOCATION_ONFIELD,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c79552283.tgfilter,tp,LOCATION_ONFIELD,0,1,1,nil)
