...
function s.matfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x18c) and c:IsType(TYPE_XYZ)
		and Duel.IsExistingMatchingCard(s.matfilter2,tp,LOCATION_GRAVE+LOCATION_DECK+LOCATION_REMOVED,0,1,nil)
end
...
function s.matop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToChain() then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.matfilter2),tp,LOCATION_GRAVE+LOCATION_DECK+LOCATION_REMOVED,0,1,1,nil,tc)
		if #g>0 then
			Duel.Overlay(tc,g)
		end
	end
