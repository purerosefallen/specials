...
function c75457624.cfilter(c)
	return ((c:IsFaceup() and c:IsLocation(LOCATION_MZONE)) or c:IsLocation(LOCATION_HAND)) and c:IsSetCard(0xb5) and c:IsAbleToRemoveAsCost()
end
...
function c75457624.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local cg=Duel.GetMatchingGroup(c75457624.cfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,nil)
	if chk==0 then
