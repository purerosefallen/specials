...
function c95209656.initial_effect(c)
	...
	e5:SetTarget(c95209656.thtg)
	e5:SetOperation(c95209656.thop)
	c:RegisterEffect(e5)
end
...
function c95209656.thfilter(c)
	return not c:IsCode(95209656) and c:IsSetCard(0x154) and c:IsAbleToHand()
end
function c95209656.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c95209656.thfilter,tp,LOCATION_DECK,0,1,nil) end
...
function c95209656.thop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c95209656.thfilter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
		...
		if Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			...
			and Duel.SelectYesNo(tp,aux.Stringid(95209656,3)) then
			Duel.BreakEffect()
