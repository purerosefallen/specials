...
function c21113684.initial_effect(c)
	...
	e6:SetDescription(aux.Stringid(21113684,3))
	...
	e6:SetCondition(c21113684.negcon)
	e6:SetTarget(c21113684.negtg)
	e6:SetOperation(c21113684.negop)
	c:RegisterEffect(e6)
end
...
function c21113684.negcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return not c:IsStatus(STATUS_BATTLE_DESTROYED) and ep~=tp and re:IsActiveType(TYPE_SPELL) and Duel.IsChainNegatable(ev)
end
function c21113684.negtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_NEGATE,eg,1,0,0)
end
function c21113684.negop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
