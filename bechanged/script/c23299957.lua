...
--kazudeyigesucai
function c23299957.filter0(c)
	return c:IsSetCard(0x16) and c:IsType(TYPE_MONSTER) and c:IsCanBeFusionMaterial() and c:IsAbleToGrave()
end
--
...
function c23299957.cfilter(c)
	return c:IsSetCard(0x16) and c:IsFaceup()
end
function c23299957.fcheck(tp,sg,fc)
...
function c23299957.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		if Duel.IsExistingMatchingCard(c23299957.cfilter,tp,LOCATION_MZONE,0,1,nil) then
			local mg2=Duel.GetMatchingGroup(c23299957.filter0,tp,LOCATION_DECK,0,nil)
			if mg2:GetCount()>0 then
				mg1:Merge(mg2)
				aux.FCheckAdditional=c23299957.fcheck
...
function c23299957.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		...
		mg3=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c23299957.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf,chkf)
	end
