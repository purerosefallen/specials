...
oneTimeSkill(74677422, function(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetCondition(function(te,...) 
		local ttp=te:GetHandlerPlayer()
		return Duel.GetLP(ttp) <= 0 
	end)
	e1:SetOperation(function(te,...)
		...
		ge2:SetOperation(function(ce,...)
			local ctp=ce:GetHandlerPlayer()
			Duel.SetLP(ctp,1) 
			ge1:Reset()
		end)
		Duel.RegisterEffect(ge2,ttp)


		local function skipcon(ge)
			return Duel.GetTurnCount()~=ge:GetLabel()
		end
		local ae1=Effect.CreateEffect(rc)
		ae1:SetType(EFFECT_TYPE_FIELD)
		ae1:SetCode(EFFECT_CHANGE_DAMAGE)
		ae1:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
		ae1:SetTargetRange(1,0)
		ae1:SetValue(0)
		ae1:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(ae1,ttp)
		local ae2=ae1:Clone()
		ae2:SetCode(EFFECT_NO_EFFECT_DAMAGE)
		ae2:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(ae2,ttp)

		local ph=Duel.GetCurrentPhase()
        local ce1=Effect.CreateEffect(rc)
        ce1:SetType(EFFECT_TYPE_FIELD)
        ce1:SetCode(EFFECT_SKIP_BP)
        ce1:SetProperty(EFFECT_FLAG_PLAYER_TARGET+EFFECT_FLAG_OATH)
        ce1:SetTargetRange(1,0)
        if Duel.GetTurnPlayer()==ttp and ph>PHASE_MAIN1 and ph<PHASE_MAIN2 then
            ce1:SetLabel(Duel.GetTurnCount())
            ce1:SetCondition(skipcon)
            ce1:SetReset(RESET_PHASE+PHASE_BATTLE+RESET_SELF_TURN,2)
        else
            ce1:SetReset(RESET_PHASE+PHASE_BATTLE+RESET_SELF_TURN,1)
        end
        Duel.RegisterEffect(ce1,ttp)
        e1:Reset()
	end
	)
	Duel.RegisterEffect(e1,tp)
