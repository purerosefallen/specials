...
function c4290468.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsControler(1-tp) then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetOperation(c4290468.rstop)
...
function c4290468.daop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not c:IsImmuneToEffect(e) then
		...
		e_reset:SetCountLimit(1)
		e_reset:SetLabelObject(elast)
		e_reset:SetCondition(c4290468.rstcon)
		e_reset:SetOperation(c4290468.rstop)
		Duel.RegisterEffect(e_reset,tp)
	end
end
function c4290468.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c4290468.rstop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
	local ecur = e:GetLabelObject()
	local tc = ecur:GetHandler()
	local elast = nil
