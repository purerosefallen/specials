...
function c56916805.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if val~=0 and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c56916805.rstcon)
			e_reset:SetOperation(c56916805.rstop)
			Duel.RegisterEffect(e_reset,tp)
		end
	end
end

function c56916805.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c56916805.rstop(e,tp,eg,ep,ev,re,r,rp)
