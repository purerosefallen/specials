...
function c88032368.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsPosition(POS_FACEUP_ATTACK) then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c88032368.rstcon)
			e_reset:SetOperation(c88032368.rstop)
			Duel.RegisterEffect(e_reset,tp)
		end
	end
end

function c88032368.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c88032368.rstop(e,tp,eg,ep,ev,re,r,rp)
