...
function c67030233.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) then
		if not tc:IsImmuneToEffect(e) then
			...
			tc:RegisterFlagEffect(67030233,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1,fid)
			if c:IsRelateToEffect(e) and c:IsFaceup() then
				if not c:IsImmuneToEffect(e) then
					...
					count = count + 1
					c:RegisterEffect(e3)
					c:RegisterFlagEffect(67030233,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1,fid)
				end
			end
		end
	end
	if elast ~= nil then
		...
		e_reset:SetCondition(c67030233.descon)
		e_reset:SetOperation(c67030233.rstop)
...
function c67030233.desfilter(c,fid)
	return c:GetFlagEffectLabel(67030233)==fid
end
...
function c67030233.descon(e,tp,eg,ep,ev,re,r,rp)
	...
	g = g:GetLabelObject()
	if not g:IsExists(c67030233.desfilter,1,nil,e:GetLabel()) then
		g:DeleteGroup()
		e:Reset()
		return false
...
function c67030233.rstop(e,tp,eg,ep,ev,re,r,rp)
	local sg = e:GetLabelObject()
	while sg:GetLabel() ~= 0 do
		sg = sg:GetLabelObject()
	end
	sg = sg:GetLabelObject()
	local dg=sg:Filter(c67030233.desfilter,nil,e:GetLabel())
	sg:DeleteGroup()
