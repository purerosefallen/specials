...
function c90246973.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 then
		...
		if not ec:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c90246973.rstcon)
			e_reset:SetOperation(c90246973.rstop)
			Duel.RegisterEffect(e_reset,tp)
		end
	end
end
function c90246973.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c90246973.rstop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
	local ecur = e:GetLabelObject()
	local tc = ecur:GetHandler()
	local elast = nil
