...
function c30604579.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if not tc:IsImmuneToEffect(e) then
			...
			tc:RegisterFlagEffect(30604579,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1,fid)
		end
		tc=g:GetNext()
	end
	...
	if elast ~= nil then
		...
		e_reset:SetCondition(c30604579.descon)
		e_reset:SetOperation(c30604579.rstop)
		Duel.RegisterEffect(e_reset,tp)
	end
end

function c30604579.desfilter(c,fid)
	return c:GetFlagEffectLabel(30604579)==fid
end

function c30604579.descon(e,tp,eg,ep,ev,re,r,rp)
	local g = e:GetLabelObject()
	while g:GetLabel() ~= 0 do
		g = g:GetLabelObject()
	end
	g = g:GetLabelObject()
	if not g:IsExists(c30604579.desfilter,1,nil,e:GetLabel()) then
		g:DeleteGroup()
		e:Reset()
		return false
...
function c30604579.rstop(e,tp,eg,ep,ev,re,r,rp)
	local sg = e:GetLabelObject()
	while sg:GetLabel() ~= 0 do
		sg = sg:GetLabelObject()
	end
	sg = sg:GetLabelObject()
	local dg=sg:Filter(c30604579.desfilter,nil,e:GetLabel())
	sg:DeleteGroup()
