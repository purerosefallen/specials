...
function c49267971.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc1:IsRelateToEffect(e) and tc1:IsFaceup() and tc2:IsRelateToEffect(e) and tc2:IsFaceup() then
		...
		if not tc2:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c49267971.rstcon)
			e_reset:SetOperation(c49267971.rstop)
			Duel.RegisterEffect(e_reset,tp)
		end
	end
end
function c49267971.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c49267971.rstop(e,tp,eg,ep,ev,re,r,rp)
