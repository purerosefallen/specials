...
function c66165755.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c66165755.rstcon)
			e_reset:SetOperation(c66165755.rstop)
...
function c66165755.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c66165755.rstop(e,tp,eg,ep,ev,re,r,rp)
