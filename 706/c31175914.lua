...
function c31175914.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c31175914.rstcon)
			e_reset:SetOperation(c31175914.rstop)
			Duel.RegisterEffect(e_reset,tp)
		end
	end
end

function c31175914.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c31175914.rstop(e,tp,eg,ep,ev,re,r,rp)
