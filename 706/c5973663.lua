...
function c5973663.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsRace(RACE_PLANT) then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c5973663.rstcon)
			e_reset:SetOperation(c5973663.rstop)
...
function c5973663.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c5973663.rstop(e,tp,eg,ep,ev,re,r,rp)
