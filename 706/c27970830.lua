...
function c27970830.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c27970830.rstcon)
			e_reset:SetOperation(c27970830.rstop)
...
function c27970830.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c27970830.rstop(e,tp,eg,ep,ev,re,r,rp)
