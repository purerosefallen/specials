...
function c74741494.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if not tc:IsImmuneToEffect(e) then
			...
			tc:RegisterFlagEffect(74741494,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1,fid)
		end
		tc=g:GetNext()
	end
	...
	if elast ~= nil then
		...
		e_reset:SetCondition(c74741494.descon)
		e_reset:SetOperation(c74741494.rstop)
		Duel.RegisterEffect(e_reset,tp)
	end
end

function c74741494.desfilter(c,fid)
	return c:GetFlagEffectLabel(74741494)==fid
end

function c74741494.descon(e,tp,eg,ep,ev,re,r,rp)
	local g = e:GetLabelObject()
	while g:GetLabel() ~= 0 do
		g = g:GetLabelObject()
	end
	g = g:GetLabelObject()
	if not g:IsExists(c74741494.desfilter,1,nil,e:GetLabel()) then
		g:DeleteGroup()
		e:Reset()
		return false
...
function c74741494.rstop(e,tp,eg,ep,ev,re,r,rp)
	local sg = e:GetLabelObject()
	while sg:GetLabel() ~= 0 do
		sg = sg:GetLabelObject()
	end
	sg = sg:GetLabelObject()
	local dg=sg:Filter(c74741494.desfilter,nil,e:GetLabel())
	sg:DeleteGroup()
