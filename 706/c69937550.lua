...
function c69937550.repcon(e)
	...
	if c:GetLeaveFieldDest()~=0 then
		c:RegisterFlagEffect(69937550, RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_DAMAGE, 0, 1)
	end
	return c:GetFlagEffect(69937550)==0 and c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsReason(REASON_DESTROY)
end
