...
function c32710364.repcon(e)
	...
	if c:GetLeaveFieldDest()~=0 then
		c:RegisterFlagEffect(32710364, RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_DAMAGE, 0, 1)
	end
	return c:GetFlagEffect(32710364)==0 and c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsReason(REASON_DESTROY)
end
