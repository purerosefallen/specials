...
function c16430187.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c16430187.rstcon)
			e_reset:SetOperation(c16430187.rstop)
			Duel.RegisterEffect(e_reset,tp)
		end
	end
end

function c16430187.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c16430187.rstop(e,tp,eg,ep,ev,re,r,rp)
