...
function c74416224.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c74416224.rstcon)
			e_reset:SetOperation(c74416224.rstop)
			Duel.RegisterEffect(e_reset,tp)
		end
	end
end

function c74416224.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c74416224.rstop(e,tp,eg,ep,ev,re,r,rp)
