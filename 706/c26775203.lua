...
function c26775203.adchange(e,tp,eg,ep,ev,re,r,rp)
	...
	if not c:IsImmuneToEffect(e) then
		...
		e_reset:SetCondition(c26775203.rstcon)
		e_reset:SetOperation(c26775203.rstop)
		Duel.RegisterEffect(e_reset,tp)
	end
end

function c26775203.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c26775203.rstop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
	local ecur = e:GetLabelObject()
	local tc = ecur:GetHandler()
	local elast = nil
