...
function c30123142.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and c30123142.filter(tc) then
		...
		if not tc:IsImmuneToEffect(e) then
			...
			e_reset:SetCondition(c30123142.rstcon)
			e_reset:SetOperation(c30123142.rstop)
			Duel.RegisterEffect(e_reset,tp)
		end
	end
end

function c30123142.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c30123142.rstop(e,tp,eg,ep,ev,re,r,rp)
