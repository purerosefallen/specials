...
function c27863269.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not at:IsImmuneToEffect(e) then
		...
		e_reset:SetCondition(c27863269.rstcon)
		e_reset:SetOperation(c27863269.rstop)
		Duel.RegisterEffect(e_reset,tp)
	end
end
function c27863269.rstcon(e,tp,eg,ep,ev,re,r,rp)
    local ecur = e:GetLabelObject()
    local tc = ecur:GetHandler()
    return tc:GetLocation() == LOCATION_MZONE and tc:GetPosition()&POS_FACEUP ~= 0
end
function c27863269.rstop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
	local ecur = e:GetLabelObject()
	local tc = ecur:GetHandler()
	local elast = nil
