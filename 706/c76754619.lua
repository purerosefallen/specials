...
function c76754619.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetLabel()==0 then
		...
		while sc do
			if not sc:IsImmuneToEffect(e) then
				...
				sc:RegisterFlagEffect(76754619,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1,fid)
			end
			sc=g:GetNext()
		end
	else
	...
	if elast ~= nil then
		...
		e_reset:SetCondition(c76754619.descon)
		e_reset:SetOperation(c76754619.rstop)
		Duel.RegisterEffect(e_reset,tp)
	end
end

function c76754619.desfilter(c,fid)
	return c:GetFlagEffectLabel(76754619)==fid
end

function c76754619.descon(e,tp,eg,ep,ev,re,r,rp)
	local g = e:GetLabelObject()
	while g:GetLabel() ~= 0 do
		g = g:GetLabelObject()
	end
	g = g:GetLabelObject()
	if not g:IsExists(c76754619.desfilter,1,nil,e:GetLabel()) then
		g:DeleteGroup()
		e:Reset()
		return false
...
function c76754619.rstop(e,tp,eg,ep,ev,re,r,rp)
	local sg = e:GetLabelObject()
	while sg:GetLabel() ~= 0 do
		sg = sg:GetLabelObject()
	end
	sg = sg:GetLabelObject()
	local dg=sg:Filter(c76754619.desfilter,nil,e:GetLabel())
	sg:DeleteGroup()
